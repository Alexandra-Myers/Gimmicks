...
public class TeleportRandomlyOnHurtGimmick extends GenericGimmick<LivingHurtEvent> {
    ...
    public void handleTypedEvent(LivingHurtEvent event) {
        ...
        double x, y, z;
        do {
            x = event.entityLiving.posX + (event.entityLiving.getRNG().nextDouble() - 0.5D) * 8.0D;
            y = event.entityLiving.posY + 4;
            z = event.entityLiving.posZ + (event.entityLiving.getRNG().nextDouble() - 0.5D) * 8.0D;
            count++;
        } while (!teleportTo(event.entityLiving, x, y, z) && count < 6);
        if (event.source.getDamageType().equalsIgnoreCase("fall")) event.ammount *= 0.5F;
    }
}
