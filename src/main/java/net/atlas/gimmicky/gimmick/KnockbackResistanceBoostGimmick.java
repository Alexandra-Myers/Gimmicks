...
public class KnockbackResistanceBoostGimmick extends GenericGimmick<TickEvent.PlayerTickEvent> {
    ...
    public static final AttributeModifier surefootedBoost = new AttributeModifier("Surefooted Boost", 1.0, 0)
        .setSaved(false);
    ...
    public void handleTypedEvent(TickEvent.PlayerTickEvent event) {
        if (event.player.getHealth() > 10.0F) {
            IAttributeInstance knockbackResistance = event.player
                .getEntityAttribute(SharedMonsterAttributes.knockbackResistance);
            if (knockbackResistance == null) knockbackResistance = event.player.getAttributeMap()
                .registerAttribute(SharedMonsterAttributes.knockbackResistance);
            if (knockbackResistance.getModifier(surefootedBoost.getID()) == null)
                knockbackResistance.applyModifier(surefootedBoost);
        } else {
            IAttributeInstance knockbackResistance = event.player
                .getEntityAttribute(SharedMonsterAttributes.knockbackResistance);
            if (knockbackResistance == null) knockbackResistance = event.player.getAttributeMap()
                .registerAttribute(SharedMonsterAttributes.knockbackResistance);
            if (knockbackResistance.getModifier(surefootedBoost.getID()) != null)
                knockbackResistance.removeModifier(surefootedBoost);
        }
    }
